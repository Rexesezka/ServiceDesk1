...
# В production значение берётся из переменной окружения DJANGO_SECRET_KEY
SECRET_KEY = os.environ.get(
    'DJANGO_SECRET_KEY',
    'django-insecure-3!q1(@jz_ntgsnhcv+wufaa8zza@wmh5)++8wdna!oxyy6!_i5',  # dev‑ключ
)

# SECURITY WARNING: don't run with debug turned on in production!
DEBUG = os.environ.get('DJANGO_DEBUG', 'True').lower() == 'true'

# Хосты, с которых можно обращаться к бэкенду.
# В production пропиши здесь домен Render, например:
...
# Базовый список origin'ов для локальной разработки
...
# Можно передать дополнительные origin'ы через переменную окружения
# CORS_ALLOWED_ORIGINS="https://your-frontend.vercel.app,https://another.com"
...
# В dev можно разрешить всё, в prod (DEBUG=False) — отключаем
